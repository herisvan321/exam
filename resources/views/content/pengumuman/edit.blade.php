  ...
  <div class="">
    <div class="panel" style="min-height: 100vh;">
      ...
      <div class="panel-body">
          <div class="row">
            <div class="col-md-8">
              <form method="post" action="{{ route('pengumuman.update', base64_encode($datafind->id)) }}" enctype="multipart/form-data">
                @csrf
                @method('put')
                 <div class="form-group">
                  ...
                  <select class="form-control" required="required" name="tingkat_id">
                    ...
                    <option value="0" {{ $datafind->tingkat_id == 0 ? 'selected="selected"': '' }}>Semua</option>
                    @foreach($tingkat as $tingkats)
                    <option value="{{ $tingkats->id }}" {{ $datafind->tingkat_id == $tingkats->id ? 'selected="selected"': '' }}>{{ $tingkats->title }}</option>
                    @endforeach
                  </select>
                </div>
                <div class="form-group">
