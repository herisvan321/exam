    ...
    <div class="panel" style="min-height: 100vh;">
      ...
      <div class="panel-body">
          <div class="row">
            ...
            <div class="col-md-3">
              <form method="post" action="{{ route('data-ujian.store') }}">
                @csrf
                		...
                		<option disabled="disabled" style="background: #dfdfdf">{{ $mat->title }}</option>
	                		@foreach($mat->tingkatKelas as $tingkelas)
	                			...
	                			@foreach($tingkelas->kelasMatpel as $kelmatpel)
	                				<option value="{{ $kelmatpel->id }}">{{ $kelmatpel->title }}</option>
	                			@endforeach
	                		@endforeach
                		@endforeach
                	</select>
                ...
                <div class="form-group">
                	...
                	<input type="number" name="jlm_soal" id="jlm_soal" placeholder="Enter Jumlah Soal ..." class="form-control" value="{{ old('jlm_soal') }}" />
                </div>
                <div class="form-group">
